...
5. 打开 opencode，按照步骤，依次执行 `/lsdd-constitution`, `/lsdd-requirement`, `/lsdd-design`, `/lsdd-testing-standards`, `/lsdd-tasks`, `/lsdd-execute`。来完成一个feature的编写。
...
### 制定项目规范
`/lsdd-constitution <用户意图>` : 智能体会根据你的意图，初始化项目，并与你讨论各种开发规范和细节，最终制定一份 `.light-spec-rules/constitution.md` 文件，之后的每个命令，都会先阅读这份文件，再执行。
...
### bug 修复
`lsdd-fix <spec-file>.md <bug描述>` : 若程序出问题了，调用 lsdd-fix ，让AI阅读上下文以及spec文档，然后进行修复，并记录日志
