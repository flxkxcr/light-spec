# 轻量规范驱动开发 (LSDD, Lightweight Spec-Drivened Development)
...
2. 将 `.light-spec-rules` 文件复制到你的项目的根目录中
3. 新建 `.light-spec` 目录
...
- 每个spec文档，都有 dependencies 字段，用于人工标记feature之间的依赖关系，智能体在读spec文件时，回递归向上溯源整个依赖链。
...
### 制定项目规范 (TODO)
`/lsdd-constitution <用户意图>` : 智能体会根据你的意图，与你讨论各种开发规范和细节，最终制定一份 `constitution.md` 文件，之后的每个命令，都会先阅读这份文件，再执行。
